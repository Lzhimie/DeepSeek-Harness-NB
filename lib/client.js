...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    const cssText = [
      ...
      // it below the caption-button band (52px, clear of the 44px win32
      // title-bar drag region) and mark it no-drag so the button stays
      // clickable even if it ever lands inside the drag region; push the
      // panel content (tab bar + explorer header) down by the same offset.
      "[class*='W-zNGW_toggleCluster']{top:52px !important;-webkit-app-region:no-drag !important}",
      "[class~='W-zNGW_pane']{padding-top:52px !important}",
